...
Paste only the block after `---` into project instructions. The builder appends a compact router.
...
Before answering, use the router below. Open the named `FAMILY_*.md`, jump to `# Skill: <name>`, and
follow that procedure. Flattened references appear as `## Reference: <path>`. `CONTROL.md` contains
global policy, reusable prompts, and governing VIP rules. If nothing matches, say so and answer
directly.
...
Use the connected GitHub repo for corpora; cite what you read and report access failures. Never browse
then substitute a recalled claim.
